...
function merge_callpeak {
    merged_input="${work}/peakcalling/merged_SEM.bam"
    rm -rf ${work}/peakcalling/merged
    mkdir ${work}/peakcalling/merged
    output_dir="${work}/peakcalling/merged"

     bsub -P acc_oscarlr -q premium -n 2 -W 24:00 -R "rusage[mem=16000]" -o "${work}/merged_peaks_job.txt" \
        "macs2 callpeak -t ${merged_input} -f BAMPE -n ${output_dir}/merged_SEM_peak -g hs --keep-dup all"
}

function count_peaks {
    narrowpeak_file="${work}/peakcalling/merged/merged_SEM_peak_peaks.narrowPeak"  # Use correct path
    output_file="${work}/peakcalling/merged/peak_counts_hg38.txt"
